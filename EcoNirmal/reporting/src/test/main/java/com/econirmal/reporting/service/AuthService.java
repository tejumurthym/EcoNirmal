...
public class AuthService {
    ...
    public String register(RegisterRequest request) {
        ...
        // Workers need admin approval; citizens auto-approved
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (user.getRole().equals("WORKER") && !user.isApproved()) {
            return new LoginResponse("Your worker account is pending admin approval", null, null, null);
        }

        return new LoginResponse("Login successful", user.getRole(), user.getName(), user.getId());
    }
}